...
print(df.loc['A','c1']) #1. A행 c1 컬럼 4


print("2. A,B행 c1 컬럼", end=" ")
print(df.loc[['A','B'],'c1']) #2. A,B행 c1 컬럼 A    4 B    5 Name: c1, dtype: int64 => series로 도출된다.
...
print(df.loc["B":"D",['c1','c3']]) #=> 행라벨로 조회하는 것이기 때문에 stop값도 포함이 된다.
'''
4. B~D행 c1,c3 컬럼
   c1  c3
B   5  50
...
print(df.loc["B":"D",'c2':'c3']) #=> 행라벨로 조회하는 것이기 때문에 stop값도 포함이 된다.
'''
5. B~D행 c2~c3 컬럼
   c2  c3
B  35  50
...
dict_value =   {'c1': [4,5,6,7,8]
                ...
                ,'c3': [43,50,61,3,56]}
df  = pd.DataFrame(dict_value, index=list('ABCDE'))
print(df.loc[[True,False,True,False,True],'c2':'c3']) #=> boolean 색인 시에는 boolean 값 개수가 행라벨 수와 일치해야한다.
...
print(df.loc[[True,False,True,False,True],[False,True,True]])
'''
7. A,C,E행 c2,c3 컬럼
   c2  c3
A  14  43
